    ...
    function salvarMembro() {
      ...
      .then(function(data) {
        if (data.success) {
          ...
          document.getElementById('cadInscricao').value = '';
          document.getElementById('cadCPF').value = '';
          document.getElementById('cadRG').value = '';
          document.getElementById('cadCurso').value = '';
          document.getElementById('cadCarga').value = '';
...
app.post('/api/login', async (req, res) => {
  ...
  if (email === 'admin@dpcrim.org' && senha === 'admin') {
    await registrarLog(email, 'Login efetuado', 'Acesso autenticado com sucesso');
...
app.post('/api/membros', async (req, res) => {
  ...
  await Membro.findOneAndUpdate({ cpfLimpo }, dadosMembro, { upsert: true, new: true });
  await registrarLog(data.operador || 'ADMIN', 'Membro Cadastrado', `Nome: ${data.nome} | CPF: ${dadosMembro.cpfMascarado}`);

  const qrCodeApi = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(urlValidacao)}`;
  res.json({ success: true, membro: dadosMembro, qrCode: qrCodeApi });
});
...
app.get('/api/filiados/buscar', async (req, res) => {
  ...
  const membro = await Membro.findOne({ cpfLimpo });
